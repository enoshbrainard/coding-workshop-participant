...
def get_members():
    ...
    return {"statusCode": 200, "headers": {"Access-Control-Allow-Origin": "*"}, "body": json.dumps(members)}
...
def create_member(body):
    with get_db() as conn:
        with conn.cursor() as cur:
            ...
            conn.commit()
    return {"statusCode": 201, "headers": {"Access-Control-Allow-Origin": "*"}, "body": json.dumps({"id": member_id})}

def get_non_direct_staff():
    # Non-direct leader / staff
    query = """
...
def get_org_leader_teams():
    # Teams reporting to organization leader
    query = """
